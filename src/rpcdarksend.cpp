...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <simplicityaddress> <amount>\n"
            "simplicityaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "darksend <simplicityaddress> <amount>\n"
            "simplicityaddress, denominate, or auto (AutoDenominate)"
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  start        - Start masternode configured in simplicity.conf\n"
                "  start-alias  - Start single masternode by assigned alias configured in masternode.conf\n"
                "  start-many   - Start all masternodes configured in masternode.conf\n"
                "  status       - Print masternode status information\n"
                "  stop         - Stop masternode configured in simplicity.conf\n"
                ...
                "  vote-many    - Vote on a Simplicity initiative\n"
                "  vote         - Vote on a Simplicity initiative\n"
                );
    ...
    if (strCommand == "genkey")
    {
        ...
        return CSimplicitySecret(secret).ToString();
    }
