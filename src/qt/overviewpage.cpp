...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeSimplicityAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeSimplicityAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeSimplicityAmount = strAnonymizeSimplicityAmount.remove(strAnonymizeSimplicityAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSimplicityAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeSimplicityAmount*COIN) nMaxToAnonymize = nAnonymizeSimplicityAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeSimplicityAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeSimplicityAmount));
        strAnonymizeSimplicityAmount = strAnonymizeSimplicityAmount.remove(strAnonymizeSimplicityAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSimplicityAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
