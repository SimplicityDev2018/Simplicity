...
//
// Bootup the masternode, look for a 200000 SPL input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in simplicity.conf */
...
{
    CSimplicityAddress address(collateralAddress);
